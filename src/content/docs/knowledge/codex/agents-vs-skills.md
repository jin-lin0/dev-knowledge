...
知识库既会从其他仓库触发维护，又有自己的目录、导航和构建规则，因此只放一个全局 `AGENTS.md` 容易出现两种问题：要么全局文件塞入过多仓库细节，要么缺少足够具体的信息架构约束。

| 层级 | 应保存什么 | 不应保存什么 |
| --- | --- | --- |
| `~/.codex/AGENTS.md` | 跨项目触发条件、知识库位置、持续授权、隐私与 Git 边界 | 具体目录清单和长篇编辑手册 |
| 知识库根目录 `AGENTS.md` | 领域分类、页面归位、导航维护和项目验收命令 | 其他仓库也必须承担的通用触发逻辑 |
| `dev-knowledge` Skill | 查重、核验、选择速记或专题、迁移与交付流程 | 每次任务都无条件生效的全局规则 |
| `writing-guide.md` | 面向维护者的完整内容规范和页面示例 | Codex 的发现与触发机制 |

官方文档说明 Codex 会先加载全局指令，再沿项目目录逐级加载更靠近当前工作的 `AGENTS.md`；因此仓库专属的信息架构和检查命令放在知识库根目录，比继续膨胀全局规则更合适：[使用 AGENTS.md 自定义指令](https://learn.chatgpt.com/zh-Hans/docs/agent-configuration/agents-md)。

但跨项目维护还有一个边界：当当前工作目录在别的仓库时，知识库根目录的 `AGENTS.md` 不会自然进入那次任务的指令链。因此全局规则仍要负责触发 Skill，而 Skill 必须主动读取知识库的写作规范和分类入口。四层需要职责互补，不能假设只要创建了仓库级 `AGENTS.md`，跨项目任务就一定会自动看到它。
