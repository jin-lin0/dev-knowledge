...
知识库按“稳定领域 → 问题主题 → 具体结论”组织，不按问答时间或任务来源归档。

### 一级主题目录

一个 `knowledge/<category>/` 目录只表达一个稳定知识领域，例如 CSS、前端运行时、Git、后端与数据、工程工具与系统。判断归属时看知识直接解释的对象：

- 样式、排版、布局和滚动机制归入 CSS。
- 框架组件、浏览器运行时、请求和资源加载归入前端。
- API、数据库、采集与服务端任务可靠性归入后端与数据。
- 包管理器、进程、命令行、操作系统服务和构建脚本归入工程工具与系统。

跨领域并不意味着复制。把完整内容放到主要对象所属的主题，其他页面只写当前上下文需要的摘要并链接过去。

### 分类首页

每个一级主题都维护 `index.md`，它不是文件清单，而是阅读地图：

- 按用户要解决的问题或学习顺序分组。
- 说明该主题与相邻主题的边界。
- 新增、移动或提升文章时同步更新入口。
- 标题使用读者会搜索的任务或概念，不使用“杂项一”“其他”等容器名。

### 速记页

速记页可以持续增长，但内部必须按问题域设置二级标题，再把具体条目放在三级标题下。新增条目时插入相应分组，不按时间简单追加到文件末尾。

出现以下任一情况时，应考虑把速记条目提升为专题文档：

- 需要完整心智模型、对照表和多个关联属性才能解释。
- 已经包含多个独立小节、完整操作流程或验证清单。
- 其他页面会频繁引用，单独页面能降低重复维护成本。

提升后，速记页只保留一句结论或导航链接；不要同时维护两份完整正文。
...
1. 保留所有仍然有价值且已经验证的结论。
2. 更新分类首页、站点首页和正文中的内部链接。
3. 搜索旧标题、旧路径和重复关键词，确认没有孤立入口或重复全文。
4. 运行内容检查和完整构建，确认路由、导航与搜索索引正常。
5. 保持 Git 暂存区不变，让所有整理结果留在未暂存修改中供人工检查。
